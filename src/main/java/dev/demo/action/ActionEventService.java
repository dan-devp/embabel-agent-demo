...
public class ActionEventService {
    ...
    public void publishWaiting(String jobId, String summary, String emailBody) {
        Map<String, Object> event = new LinkedHashMap<>();
        event.put("type", "step");
        event.put("stepId", "approval_gate");
        event.put("stepName", "Warte auf Angebot-Freigabe");
        event.put("status", StepStatus.WAITING.name());
        event.put("message", summary);
        event.put("emailBody", emailBody);
        event.put("timestamp", System.currentTimeMillis());
        send(jobId, event);
    }

    public void publishResult(String jobId, SalesResult result) {
        send(jobId, Map.of("type", "done", "result", result));
        complete(jobId);
    }

    public void publishTool(String jobId, String toolId, String toolName, StepStatus status) {
        Map<String, Object> event = new LinkedHashMap<>();
        event.put("type", "tool");
